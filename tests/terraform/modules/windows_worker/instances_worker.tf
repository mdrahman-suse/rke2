resource "aws_instance" "windows_worker" {
  ...
  user_data              = templatefile("join_rke2_windows_agent.tftpl", {serverIP: "${local.master_ip}", token: "${local.node_token}", version: "${var.rke2_version}"})
  subnet_id              = var.subnets
  availability_zone      = var.availability_zone
  vpc_security_group_ids = ["${var.sg_id}"]
  key_name               = var.ssh_key
  tags = {
    Name = "${var.resource_name}-win-worker"
